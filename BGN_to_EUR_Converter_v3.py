...
def toggle_rate_lock():
    if entry_rate["state"] == "normal":
        entry_rate.configure(state="disabled")
        btn_lock.configure(text="🔒")
    else:
        entry_rate.configure(state="normal")
        btn_lock.configure(text="🔓")
    convert()  # Recalculate with updated rate if changed
